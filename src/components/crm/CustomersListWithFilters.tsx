...
export function CustomersListWithFilters({ userDepartment, onViewCustomer }: CustomersListWithFiltersProps) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log('[CustomersListWithFilters] Failed to fetch users - server returned error');
        setUsers([]);
      }
    } catch (error) {
      console.log('[CustomersListWithFilters] Backend not available for users fetch');
      setUsers([]);
    }
  };
  ...
  const fetchCustomers = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(`[CustomersListWithFilters] ✅ Successfully loaded ${result.data.length} customers`);
      } else {
      ...
      // Set empty customers array so UI shows "No customers found" instead of infinite loading
      setCustomers([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDeleteCustomer = async (customerId: string, customerName: string) => {
    ...
    try {
      const response = await fetch(`${API_URL}/clients/${customerId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${publicAnonKey}`,
        },
  ...
  const getContactCount = (customerId: string, customerName: string): number => {
    ...
    if (customerId === 'CUST-001') {
      ...
      console.log(`[DEBUG] All contacts:`, contacts.map(c => ({ id: c.id, name: c.name, customer_id: c.customer_id })));
    }
    return count;
  };

  // Get latest activity date for each customer - activities removed
  const getLastActivityDate = (customerId: string): string | null => {
    // Since activities are removed, return null for now
    return null;
  };
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[13px]" style={{ color: "var(--neuron-ink-secondary)" }}>
                        {getContactCount(customer.id, customer.name || customer.company_name || '')}
                      </span>
                    </div>
