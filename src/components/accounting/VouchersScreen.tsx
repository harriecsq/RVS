...
export function VouchersScreen() {
  ...
  const filteredVouchers = (statusFilter.length > 0 || payeeFilter.length > 0 || portFilter.length > 0 || clientSelections.length > 0)
    ? [...filteredVouchersRaw].sort((a, b) => {
        ...
        if (payeeFilter.length > 0) {
          const ai = payeeFilter.indexOf(a.payee || "");
          const bi = payeeFilter.indexOf(b.payee || "");
          const d = (ai === -1 ? Number.MAX_SAFE_INTEGER : ai) - (bi === -1 ? Number.MAX_SAFE_INTEGER : bi);
          if (d !== 0) return d;
        }
        if (portFilter.length > 0) {
        ...
        if (clientSelections.length > 0) {
          const aIdx = clientSelections.findIndex((sel) =>
            clientSelectionMatches([sel], { company: a.consignee || "", client: a.shipper || "" })
          );
          const bIdx = clientSelections.findIndex((sel) =>
            clientSelectionMatches([sel], { company: b.consignee || "", client: b.shipper || "" })
          );
    ...
    },
    {
      ...
      cell: (voucher) => {
        const linkedRef =
          ...
          (voucher.bookingId ? bookingEnrichMap.get(voucher.bookingId)?.bookingNumber : "") ||
          voucher.bookingId ||
          "";
        if (!linkedRef) {
      ...
      <div style={{ padding: "0 48px 24px 48px" }}>
        ...
        <div style={{
          ...
          <MultiSelectPortalDropdown
            ...
            options={(Array.from(new Set(vouchers.map(v => v.payee).filter(Boolean))).sort() as string[]).map(p => ({ value: p, label: p }))}
