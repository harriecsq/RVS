...
export function BillingsScreen() {
  ...
  const filterBillings = () => {
    ...
    if (statusFilter.length > 0 || portFilter.length > 0 || clientSelections.length > 0) {
  ...
  const columns: ColumnDef<Billing>[] = [
    ...
    },
    {
      header: "Created",
      cell: (b) => {
        const d = b.billingDate || b.created_at;
        return <div style={{ fontSize: "13px", color: "#0A1D4D" }}>{d ? new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : "—"}</div>;
      },
    },
  ];
