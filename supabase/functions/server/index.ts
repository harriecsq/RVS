...
// NOTE: These sub-path routes MUST be registered before the generic
// GET/PUT /export-bookings/:id routes, otherwise Hono matches /:id first.

// Get all documents for an export booking
...
// Create or update a specific export document
...
// Delete a specific export document
...
app.get("/make-server-ce0d67b8/next-ref/:type", async (c) => {
  try {
    ...
    if (type === "import") {
      ...
      const prefix = `${companyCode} SC ${year}-`;
      const all = await kv.getByPrefix("export_booking:");
